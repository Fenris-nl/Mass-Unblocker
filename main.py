...
# History management and Unblock action.
...
# Virtual tree view for handling large datasets.
...
# Functions to check if a file is blocked and to unblock it.
...
# Scans directories for .exe files and collects details.
...
class UnblockerApp(ttk.Window):
    def __init__(self):
        ...
        self._set_app_icon()  # Set the custom application icon
        self.title("Executable Unblocker - By Kaan Erdem / Fenris")
        self.geometry("1280x800")
        self.minsize(1024, 600)
        self.resizable(True, True)
        # Show the improved splash screen first.
        self.after_idle(self._show_splash)
    ...
    def _show_splash(self):
        ...
        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            splash_img_path = os.path.join(script_dir, "splash.png")
            # Use PIL to open the splash image
            self.splash_image_pil = Image.open(splash_img_path)
            self.splash_image_tk = ImageTk.PhotoImage(self.splash_image_pil)

            self.splash_canvas = tk.Canvas(
                self.splash_frame, highlightthickness=0)
            self.splash_canvas.pack(fill="both", expand=True)
            self.splash_image_id = self.splash_canvas.create_image(
                0, 0, anchor="nw", image=self.splash_image_tk)
            self.splash_canvas.bind("<Configure>", self._resize_splash_image)
        except Exception as e:
            logging.error(f"Failed to load splash image: {e}")
            splash_label = ttk.Label(
                self.splash_frame, text="Loading...", font=("Helvetica", 36))
            splash_label.place(relx=0.5, rely=0.5, anchor="center")
        self.after(3000, self._init_ui)

    def _resize_splash_image(self, event):
        if hasattr(self, 'splash_image_pil'):
            canvas_width = event.width
            canvas_height = event.height
            resized_img = self.splash_image_pil.resize(
                (canvas_width, canvas_height), Image.LANCZOS)
            self.splash_image_tk = ImageTk.PhotoImage(resized_img)
            self.splash_canvas.itemconfig(
                self.splash_image_id, image=self.splash_image_tk)

    def _init_ui(self):
        # Remove the splash screen.
        self.splash_frame.destroy()

        # Set fonts and initialize variables.
        ...
        # Update the directory entry widget.
    ...
    def _create_status_bar(self):
        ...
        self.progress = ttk.Progressbar(
            status_frame, mode="determinate", length=200, bootstyle=SUCCESS)
        self.progress.pack(side="right", padx=5)

    def _setup_treeview(self):
        self.tree_frame = ttk.Frame(self.left_container)
        self.tree = VirtualTreeView(self.tree_frame,
                                    columns=("File", "Size",
                                             "Modified", "Status"),
                                    show="headings",
                                    selectmode="extended",
    ...
    # ----- Icon View Setup with Dynamic Arrangement -----
    ...
    def handle_icon_inner_configure(self, event):
        ...
        if abs(current_width - self.last_icon_inner_width) > 10:
            self.arrange_icons()
    ...
    def populate_icon_view(self, records: List[Tuple[str, str, str, str, str]]):
        ...
        for record in records:
            ...
            btn = ttk.Button(self.icon_inner,
    ...
    def arrange_icons(self):
        ...
        icon_width = 170  # Adjusted for button width + padding
    ...
    def show_about(self):
        ...
        # Set icon for the About window using iconbitmap
        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            icon_path = os.path.join(script_dir, "app_icon.ico")
            about_window.iconbitmap(icon_path)
        except Exception as e:
            logging.error(f"Failed to load about window icon: {e}")

        # Title and description
        title_label = ttk.Label(
            about_window, text="Executable Unblocker - By Kaan Erdem", font=("Segoe UI", 14, "bold"))
        title_label.pack(pady=(10, 5))

        version_label = ttk.Label(
            about_window, text="Version: 1.0.0", font=("Segoe UI", 10))
        version_label.pack(pady=5)

        desc_label = ttk.Label(
            about_window, text="A tool to scan and unblock executables.", font=("Segoe UI", 10))
        desc_label.pack(pady=5)

        dev_label = ttk.Label(
            about_window, text="Developed by: Kaan Erdem", font=("Segoe UI", 10))
        dev_label.pack(pady=5)

        # Create clickable email link using a tk.Label for styling flexibility.
        email_label = tk.Label(about_window, text="Email: kaanerdem3@gmail.com",
                               font=("Segoe UI", 10), fg="blue", cursor="hand2")
        email_label.pack(pady=5)
        email_label.bind(
            "<Button-1>", lambda e: webbrowser.open("mailto:kaanerdem3@gmail.com"))

        # Create clickable GitHub link.
        github_label = tk.Label(about_window, text="GitHub: https://github.com/Fenris-nl",
                                font=("Segoe UI", 10), fg="blue", cursor="hand2")
        github_label.pack(pady=5)
        github_label.bind(
            "<Button-1>", lambda e: webbrowser.open("https://github.com/Fenris-nl"))

        thanks_label = ttk.Label(
            about_window, text="Thank you for using this application!", font=("Segoe UI", 10))
        thanks_label.pack(pady=10)

        close_btn = ttk.Button(about_window, text="Close",
                               command=about_window.destroy, bootstyle=PRIMARY)
    ...
    # For icon view selection toggle.
    def toggle_icon_selection(self, file_path: str):
        current = self.icon_selected.get(file_path, False)
        self.icon_selected[file_path] = not current
        btn = self.icon_buttons.get(file_path)
        if btn:
            base_name = os.path.basename(file_path)
            if self.icon_selected[file_path]:
                btn.configure(text=f"✔ {base_name}", bootstyle="primary")
            else:
                btn.configure(text=base_name, bootstyle=DANGER if is_file_blocked(
                    file_path) else SUCCESS)
        self.apply_filter()
